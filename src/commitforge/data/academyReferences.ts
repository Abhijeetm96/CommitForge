...
export const ALL_CONCEPT_REFERENCES: Record<string, ConceptReferenceSupplement> = {
  ...
  'c-actions-matrix': {
    ...
    options: [
      { flag: 'fail-fast: false', description: 'Prevents cancelling siblings when one matrix cell fails.' },
      { flag: 'max-parallel: <int>', description: 'Caps number of concurrently executing matrix runners.' },
      { flag: 'include', description: 'Appends customized parameter combinations into matrix.' },
      { flag: 'exclude', description: 'Removes unsupported combinations from generated matrix grid.' },
    ],
    syntaxCheatSheet: [
      'strategy.matrix : Multiplies parameter arrays into parallel runners',
      'strategy.fail-fast: false : Allows remaining jobs to run if one fails',
      'strategy.max-parallel: 4 : Caps simultaneous concurrent runners',
      'matrix.include : Adds specific permutations or custom variables',
    ],
    commonErrors: [
    ...
    gitInternals: {
      objectType: 'Workflow Execution Strategy Matrix',
      explanation: 'Multiplies dimension arrays into a set of discrete job instances executed concurrently across isolated virtual machines.',
      storageLocation: '.github/workflows/*.yml and GitHub Actions scheduler queue',
    },
  },
  ...
  'c-actions-caching': {
    ...
    options: [
      { flag: 'path', description: 'Runner directory or file paths to preserve across workflow executions.' },
      { flag: 'key', description: 'Unique primary cryptographic hash key for cache persistence.' },
      { flag: 'restore-keys', description: 'Ordered list of prefix keys used if primary key misses.' },
    ],
    syntaxCheatSheet: [
      'actions/cache@v4 : Saves and restores directories across workflow runs',
      'key: ${{ runner.os }}-node-${{ hashFiles(...) }} : Cryptographic hash cache key',
      'restore-keys : Fallback prefixes to restore partial cache if exact key misses',
      'actions/upload-artifact@v4 : Preserves build files for subsequent jobs',
    ],
    commonErrors: [
    ...
    gitInternals: {
      objectType: 'Content-Addressable Dependency Cache Store',
      explanation: 'Saves tarball archives of package directories into cloud cache storage indexed by SHA-256 lockfile hashes.',
      storageLocation: 'GitHub Actions cloud cache storage backend (10GB quota per repository)',
    },
  },

  'c-actions-reusable': {
    officialDocUrl: 'https://docs.github.com/en/actions/using-workflows/reusing-workflows',
    synopsis: 'uses: <owner>/<repo>/.github/workflows/<workflow>.yml@<ref>\nwith: ...\nsecrets: inherit',
    options: [
      { flag: 'workflow_call', description: 'Event trigger defining the workflow as callable by other caller workflows.' },
      { flag: 'secrets: inherit', description: 'Passes all secrets from caller repository to called workflow automatically.' },
      { flag: 'inputs', description: 'Declares strongly-typed inputs accepted from caller workflows.' },
    ],
    syntaxCheatSheet: [
      'on: workflow_call : Exposes workflow to be called by other workflows',
      'uses: org/repo/.github/workflows/reusable.yml@v1 : Invokes reusable workflow',
      'secrets: inherit : Passes all secrets from caller to called workflow',
      'using: "composite" : Bundles multiple action steps into a single reusable action',
    ],
    commonErrors: [
    ...
    gitInternals: {
      objectType: 'Centralized Reusable Workflow Routine',
      explanation: 'Compiles caller and called workflow definitions into a unified execution DAG with security boundary checks.',
      storageLocation: 'Shared organization repository under `.github/workflows/`',
    },
  },
  ...
  'c-actions-gitops': {
    ...
    synopsis: 'runs-on: [self-hosted, arc-runner] | argocd app sync <app> --prune',
    options: [
      { flag: 'runs-on: self-hosted', description: 'Directs jobs to private enterprise runners inside corporate VPC networks.' },
      { flag: 'permissions: id-token: write', description: 'Enables keyless cloud authentication via OIDC identity federation.' },
      { flag: 'environment', description: 'Attaches deployment gates, protection rules, and required manual sign-offs.' },
    ],
    syntaxCheatSheet: [
      'runs-on: [self-hosted, arc-runner] : Routes execution to private cluster runner',
      'permissions: id-token: write : Enables OIDC keyless cloud authentication',
      'ghcr.io/org/app:${{ github.sha }} : Packages immutable OCI container image',
      'argocd app sync <app> --prune : Triggers immediate GitOps pull reconciliation',
    ],
    commonErrors: [
    ...
    gitInternals: {
      objectType: 'GitOps Declarative Reconciler & OCI Pipeline',
      explanation: 'Builds OCI container images and updates Git manifest repositories, triggering in-cluster ArgoCD pull reconciliation.',
      storageLocation: 'Git manifest repository, GitHub Packages (GHCR), and target Kubernetes cluster',
    },
  },
