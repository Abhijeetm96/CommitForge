...
export const ALL_PRACTICE_CHALLENGES: Record<string, ConceptPracticeChallenge> = {
  ...
  'c-actions-matrix': {
    title: 'Configure Parallel Multi-OS Matrix Pipeline',
    objective: 'Define a build matrix testing Node.js 18 and 20 across Ubuntu and Windows runners.',
    instructions: 'Stage `.github/workflows/matrix.yml` and commit with conventional commit message.',
    startingState: 'Matrix workflow file defined with strategy matrix in `.github/workflows/matrix.yml`.',
    goalState: 'Parallel matrix build workflow committed to repository.',
    seedCommands: ['git init', 'mkdir -p .github/workflows'],
    initialFiles: {
      '.github/workflows/matrix.yml': 'name: Multi-OS Matrix\non: [push]\njobs:\n  test:\n    strategy:\n      fail-fast: false\n      matrix:\n        os: [ubuntu-latest, windows-latest]\n        node: [18, 20]\n    runs-on: ${{ matrix.os }}\n    steps:\n      - uses: actions/checkout@v4\n      - run: npm test\n',
    },
    expectedCommands: ['git add .github/workflows/matrix.yml', 'git commit -m "ci(matrix): configure parallel multi-os test matrix"'],
    hints: [
      'Stage matrix workflow: `git add .github/workflows/matrix.yml`.',
      'Commit: `git commit -m "ci(matrix): configure parallel multi-os test matrix"`.',
    ],
    solutionExplanation: 'Matrix builds multiply operating system and runtime version dimensions into concurrent parallel jobs.',
    safeFailure: {
      mistakeTitle: 'Exceeding Runner Concurrency Quota',
      mistakeCommand: 'matrix with 50 permutations',
      whatHappened: 'Created more jobs than permitted by account limits, queuing runs.',
      whatWasNotLost: 'Workflows remain safely queued.',
      recoveryCommand: 'strategy: max-parallel: 4',
      recoveryExplanation: 'Throttle simultaneous matrix executions with `max-parallel`.',
    },
  },

  'c-actions-caching': {
    title: 'Implement High-Speed Dependency Caching',
    objective: 'Configure actions/cache to cache node_modules via package-lock.json hash key.',
    instructions: 'Stage cached CI workflow and commit to repository.',
    startingState: 'Cached CI workflow configuration in `.github/workflows/ci.yml`.',
    goalState: 'Dependency caching workflow committed.',
    seedCommands: ['git init', 'mkdir -p .github/workflows'],
    initialFiles: {
      '.github/workflows/ci.yml': 'name: CI\non: [push]\njobs:\n  test:\n    runs-on: ubuntu-latest\n    steps:\n      - uses: actions/checkout@v4\n      - uses: actions/cache@v4\n        with:\n          path: ~/.npm\n          key: ${{ runner.os }}-node-${{ hashFiles(\'**/package-lock.json\') }}\n      - run: npm ci\n',
    },
    expectedCommands: ['git add .github/workflows/ci.yml', 'git commit -m "ci(cache): implement high-speed dependency caching"'],
    hints: [
      'Stage cache workflow: `git add .github/workflows/ci.yml`.',
      'Commit: `git commit -m "ci(cache): implement high-speed dependency caching"`.',
    ],
    solutionExplanation: 'Caching package directories dramatically reduces CI duration by eliminating redundant network downloads.',
    safeFailure: {
      mistakeTitle: 'Hardcoded Invariable Cache Key',
      mistakeCommand: 'key: npm-cache',
      whatHappened: 'Cache key never changes, causing stale dependencies to be reused indefinitely.',
      whatWasNotLost: 'Purge cache from repository settings.',
      recoveryCommand: 'key: ${{ runner.os }}-node-${{ hashFiles(\'**/package-lock.json\') }}',
      recoveryExplanation: 'Always incorporate lockfile hash in cache keys to trigger invalidation upon dependency updates.',
    },
  },

  'c-actions-reusable': {
    title: 'Invoke Reusable Organization Workflow',
    objective: 'Call a centralized security and test workflow using workflow_call specification.',
    instructions: 'Stage caller workflow `.github/workflows/caller.yml` and commit.',
    startingState: 'Caller workflow referencing enterprise reusable template.',
    goalState: 'Reusable workflow caller committed.',
    seedCommands: ['git init', 'mkdir -p .github/workflows'],
    initialFiles: {
      '.github/workflows/caller.yml': 'name: App Delivery\non: [push]\njobs:\n  call-ci:\n    uses: enterprise-org/workflows/.github/workflows/standard-ci.yml@v1\n    secrets: inherit\n',
    },
    expectedCommands: ['git add .github/workflows/caller.yml', 'git commit -m "ci(reusable): invoke enterprise standard reusable workflow"'],
    hints: [
      'Stage caller workflow: `git add .github/workflows/caller.yml`.',
      'Commit: `git commit -m "ci(reusable): invoke enterprise standard reusable workflow"`.',
    ],
    solutionExplanation: 'Reusable workflows enable organization-wide security governance and DRY pipeline templates.',
    safeFailure: {
      mistakeTitle: 'Unpinned Branch Reference',
      mistakeCommand: 'uses: org/workflows@main',
      whatHappened: 'Unpinned branch reference leaves pipeline vulnerable to upstream breaking changes.',
      whatWasNotLost: 'Pin reference to immutable tag.',
      recoveryCommand: 'uses: org/workflows@v1.2.0',
      recoveryExplanation: 'Always pin reusable workflows to semantic version tags or commit SHAs.',
    },
  },

  'c-actions-gitops': {
    title: 'Configure GitOps Container Delivery Pipeline',
    objective: 'Automate container image build and manifest repository update for ArgoCD deployment.',
    instructions: 'Stage GitOps workflow in `.github/workflows/gitops.yml` and commit.',
    startingState: 'GitOps deployment pipeline workflow created.',
    goalState: 'Declarative GitOps pipeline committed.',
    seedCommands: ['git init', 'mkdir -p .github/workflows'],
    initialFiles: {
      '.github/workflows/gitops.yml': 'name: GitOps Sync\non:\n  push:\n    branches: [main]\njobs:\n  deploy:\n    runs-on: ubuntu-latest\n    steps:\n      - uses: actions/checkout@v4\n      - run: echo "Building container and pushing to GHCR"\n',
    },
    expectedCommands: ['git add .github/workflows/gitops.yml', 'git commit -m "ci(gitops): setup declarative ArgoCD delivery pipeline"'],
    hints: [
      'Stage GitOps workflow: `git add .github/workflows/gitops.yml`.',
      'Commit: `git commit -m "ci(gitops): setup declarative ArgoCD delivery pipeline"`.',
    ],
    solutionExplanation: 'GitOps uses Git commits as the single source of truth for declared production infrastructure state.',
    safeFailure: {
      mistakeTitle: 'Deploying Direct to Production Cluster',
      mistakeCommand: 'kubectl apply -f k8s/',
      whatHappened: 'Pushed cluster mutations directly from runner without GitOps audit trail.',
      whatWasNotLost: 'Revert direct changes.',
      recoveryCommand: 'git push origin main (to manifests repo)',
      recoveryExplanation: 'Route all cluster deployments through declarative Git manifest commits.',
    },
  },
